...
(function(){
    ...
    var bannner = document.getElementById(' banner');
   // 克隆第一张
  ...
  function right_btn_handler (){
    ...
    //设置小圆点的current在谁身上，序号为idx的小圆点才有current类名，其它的li都没有类名
    function setCircles(){
        //遍历,遍历0、1、2、3、4。每遍历一个数字，都要和idx比一下，如果相等，就把这项设置类名为current，否则去掉类名。
        for(var i = 0;i <= 4;i++){
            //这里的%5非常巧妙，1、2、3、4除以5的余数都是它本身，但是5除以5等于0
    ...
    banner.onmouseleave = function(){
        //设表先关
        clearInterval(timer);
        //设置定时器，这里不加var,加var就变为局部变量，clear不掉
